<section class="w-full">
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h3 class="text-lg font-semibold text-zinc-900 dark:text-zinc-100">{{ __('Supplier Statement') }}</h3>
                <p class="text-sm text-zinc-600 dark:text-zinc-400 mt-1">{{ $supplier->name }}</p>
            </div>
            <div class="flex items-center gap-3">
                <flux:button href="{{ route('purchases.suppliers') }}" variant="ghost" icon="arrow-left" wire:navigate>
        ...
        <!-- Supplier Details -->
        <div class="bg-white dark:bg-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-700 p-4 sm:p-6 shadow-sm">
            <h4 class="text-md font-semibold text-zinc-900 dark:text-zinc-100 mb-4">{{ __('Supplier Details') }}</h4>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <p class="text-sm text-zinc-500 dark:text-zinc-400">{{ __('Name') }}</p>
                    <p class="font-medium text-zinc-900 dark:text-zinc-100">{{ $supplier->name }}</p>
                </div>
                @if($supplier->email)
                    <div>
                        <p class="text-sm text-zinc-500 dark:text-zinc-400">{{ __('Email') }}</p>
                        <p class="font-medium text-zinc-900 dark:text-zinc-100">{{ $supplier->email }}</p>
                    </div>
                @endif
                @if($supplier->phone)
                    <div>
                        <p class="text-sm text-zinc-500 dark:text-zinc-400">{{ __('Phone') }}</p>
                        <p class="font-medium text-zinc-900 dark:text-zinc-100">{{ $supplier->phone }}</p>
                    </div>
                @endif
                @if($supplier->address)
                    <div>
                        <p class="text-sm text-zinc-500 dark:text-zinc-400">{{ __('Address') }}</p>
                        <p class="font-medium text-zinc-900 dark:text-zinc-100">{{ $supplier->address }}</p>
                    </div>
                @endif
            </div>
        </div>
        ...
        @if($closingBalance > 0)
            <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
                <p class="text-sm text-red-800 dark:text-red-400">
                    <strong>{{ __('Amount Owed') }}:</strong> NAD {{ number_format($closingBalance, 2) }}
                </p>
            </div>
        @elseif($closingBalance < 0)
            <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
                <p class="text-sm text-green-800 dark:text-green-400">
                    <strong>{{ __('Overpayment') }}:</strong> NAD {{ number_format(abs($closingBalance), 2) }}
                </p>
            </div>
        @else
